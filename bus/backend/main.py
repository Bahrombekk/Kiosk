...
async def lifespan(app: FastAPI):
    ...
    log.info("Server tayyor (%s) — %s://%s:%s",
             config.VERTICAL, scheme, config.HOST, config.PORT)
    if config.DISCOVERY_ENABLED:
        discovery.start()   # LAN beacon — faqat kiosk bo'lsa (poyezd). Avtobusда o'chiq.
...
# --- Katalog ---
def _blocked_now():
    """Qurilma GLOBAL blokdami? — litsenziya yaroqsiz/muddati o'tgan, qo'lda blok
    (trial_blocked, bulut paneldan) yoki texnik rejim. WEB-ONLY avtobusда bu
    yo'lovchiga to'g'ridan-to'g'ri ta'sir qiladi: blok bo'lsa kontent/reklama/
    media endpointlari BO'SH/403 qaytaradi — o'g'irlangan yoki to'lovsiz qurilma
    sayti ham ishlamaydi (poyezdда faqat kiosk qulflanardi, web ochiq qolardi)."""
    try:
        s = db.get_settings()
        lic_blocked = licensing.state()["blocked"] or db.trial_state(s)["blocked"]
        maintenance = str(s.get("maintenance") or "0") == "1"
        return bool(lic_blocked or maintenance)
    except Exception:
        return False
...
def content_detail(content_id: int):
    if _blocked_now():
        raise HTTPException(403, "Xizmat vaqtincha to'xtatilgan")
