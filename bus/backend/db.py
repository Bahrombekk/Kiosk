...
def get_or_create_api_key():
    ...
    # Docker/env: KIOSK_API_KEY berilса — o'sha ishlatiladi (backend va web bir
    # xil kalitни .env'дан oladi). Aks holда bazadagi/yangi generatsiya.
    env_key = os.environ.get("KIOSK_API_KEY")
    if env_key:
        if get_settings().get("api_key") != env_key:
            set_setting("api_key", env_key)
        return env_key
