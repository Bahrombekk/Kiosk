...
# ═══════════════════════════════════════════════════════════════════════════
#  BO'LIM INDEKSI (navigatsiya — funksiya nomini qidiring)
#    connect / _conn / SCHEMA / init_db .......... ulanish, jadvallar, migratsiya
#    get_setting / set_setting ................... sozlamalar
#    get_user / upsert_user / hash_secret ........ foydalanuvchilar + parol
#    create_server / approve_server / update_server / *_kiosks .. serverlar
#    add_content / get_content / assign / bump_rev / desired_content .. kontent
#    add_ad / assign_ads / add_site / add_stop / replace_stops .. reklama/sayt/bekat
#    set_branding / lib_add / lib_list / lib_delete ............. brending + kutubxona
#    queue_op / due_ops / create_job / get_jobs ................ navbat + ishlar
#    insert_stats / insert_logs / add_event / stats_* .......... statistika/loglar
#  Eslatma: BARCHA so'rovlar parametrli (?), string-format faqat qattiq
#  ustun/jadval nomlari uchun. Baza SQLite (WAL).
# ═══════════════════════════════════════════════════════════════════════════
...
def init_db():
    ...
    try:
        ...
        for col in ("catalog_counts", "local_catalog"):
            if col not in scols:
                conn.execute(f"ALTER TABLE servers ADD COLUMN {col} TEXT")
        if "vertical" not in scols:
            # Vertikal: 'train' (poyezd) | 'bus' (avtobus). Panel yorliqlari
            # (Vagon/Avtobus, poyezd.uz/avtobus.uz, kiosk maydonlari...) shunga
            # qarab moslanadi. Mavjud serverlar POYEZD deb hisoblanadi.
            conn.execute("ALTER TABLE servers ADD COLUMN vertical"
                         " TEXT DEFAULT 'train'")
        ...
        # Banner kutubxonasi — vertikal bo'yicha ajratiladi (poyezd rasmlari
        # avtobusда ko'rinmasin). Mavjud bannerlar POYEZD deb belgilanadi.
        lcols = {r["name"] for r in
                 conn.execute("PRAGMA table_info(branding_library)").fetchall()}
        if "vertical" not in lcols:
            conn.execute("ALTER TABLE branding_library ADD COLUMN vertical"
                         " TEXT DEFAULT 'train'")
        conn.commit()
    finally:
        conn.close()
...
def update_server(server_id, **fields):
    allowed = ("name", "route", "note", "version", "kiosks_total", "kiosks_online",
               ...
               "license_info", "catalog_counts", "local_catalog", "vertical")
...
def lib_add(kind, sha, name, size, vertical="train"):
    """Kutubxonaga qo'shadi (bir xil rasm ikki marta yozilmaydi) va id qaytaradi.
    `vertical` — poyezd/avtobus: rasmlar shu tur serverlardagina ko'rinadi."""
    if vertical not in ("train", "bus"):
        vertical = "train"
    with _conn() as c:
        row = c.execute("SELECT id FROM branding_library WHERE kind=? AND sha=?"
                        " AND vertical=?", (kind, sha, vertical)).fetchone()
        if row:
            return row["id"]
        cur = c.execute("INSERT INTO branding_library (kind,sha,name,size,vertical)"
                        " VALUES (?,?,?,?,?)", (kind, sha, name, size, vertical))
        return cur.lastrowid


def lib_list(kind="hero", vertical=None):
    """Bannerlar ro'yxati. `vertical` berilса — faqat o'sha turdagilar
    (poyezd rasmlari avtobusда ko'rinmasin)."""
    sql = "SELECT * FROM branding_library WHERE kind=?"
    args = [kind]
    if vertical in ("train", "bus"):
        sql += " AND vertical=?"
