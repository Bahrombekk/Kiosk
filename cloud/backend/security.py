...
SIGN_KEY_PATH = config.SIGNING_KEY_PATH
...
def ensure_admin_password():
    ...
    # Login nomi .env'dan (CLOUD_ADMIN_USER), standart "admin".
    admin_user = config.ADMIN_USER_ENV or db.DEFAULT_USER
    ...
    if not config.ADMIN_PASS_ENV:
        ...
        print("\n" + "=" * 62, file=sys.stderr)
        print(f"  ADMIN LOGIN: {admin_user}", file=sys.stderr)
